...
def get_loaders(dataset, n_ex, batch_size, train_set, shuffle, data_augm, img_folder_dir=None):
    # dir_ = '~/data/' if os.path.exists('/home/maksym') else '/tmldata1/andriush/data'
    dir_ = 'data'
    # dir_ = '/tmp/experiments/data'
    num_workers = 2
    ...
    if train_set:
        if dataset == 'tinyimagenet':
            if img_folder_dir is None:
                img_folder_dir = 'data/tinyimagenet/train'
            data = datasets.ImageFolder(img_folder_dir, transform=transform)
        elif dataset != 'svhn':
        ...
        if dataset != 'tinyimagenet':
            ...
            data.samples, data.targets = data.samples[:n_ex], data.targets[:n_ex]
        ...
        if dataset == 'tinyimagenet':
            if img_folder_dir is None:
                img_folder_dir = 'data/tinyimagenet/val/images'
            data = datasets.ImageFolder(img_folder_dir, transform=transform)
        elif dataset != 'svhn':
...
datasets_dict = {'mnist': datasets.MNIST, 'svhn': datasets.SVHN, 'cifar10': datasets.CIFAR10,
                 'cifar10_binary': datasets.CIFAR10, 'cifar10_binary_gs': datasets.CIFAR10, 'cifar100': datasets.CIFAR100
                 }
shapes_dict = {'mnist': (60000, 1, 28, 28), 'svhn': (73257, 3, 32, 32), 'cifar10': (50000, 3, 32, 32),
               'cifar10_binary': (10000, 3, 32, 32), 'cifar10_binary_gs': (10000, 1, 32, 32), 'cifar100': (50000, 3, 32, 32),
               'tinyimagenet': (100000, 3, 64, 64),
